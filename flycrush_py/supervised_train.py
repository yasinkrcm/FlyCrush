#!/usr/bin/env python3
"""Supervised imitation pretrain: learns valid swaps from planner labels.

Uses ONLY board features (pair 20-dim + global 73) — planner is NEVER
consulted at inference. Valid-pair labels are the teaching signal,
exactly like Fly Dino's CEM pretrain before REINFORCE.

Usage:
    ./.venv/bin/python -m flycrush_py.supervised_train --boards 8000 --epochs 12 --lr 0.08
Writes public/data/readout-weights.json + updates training-report.json (adds supervised section).
"""
...
from flycrush_py.rl import CELL_N, DIR_N, create_policy, export_weights, extract_features, cell_features, supervised_update, _PAIR_MASK
...
def main():
    ...
    try:
        ...
        rep = {}
    rep["supervised"] = {"boards": args.boards, "epochs": args.epochs, "lr": args.lr, "hit120": round(hit,3), "avgScore120": round(avg_score,1)}
